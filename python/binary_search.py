...
def binarySearchFirst(sa: SuffixArray, value: str) -> int:
    ...
    while lo <= hi:
        mid = (lo + hi) // 2
        if text[index[mid]:index[mid] + val_len] < value:
            lo = mid + 1
        elif text[index[mid]:index[mid] + val_len] > value:
            hi = mid - 1
        elif mid - 1 >= 0 and text[index[mid]:index[mid] + val_len] == text[index[mid-1]: index[mid-1] + val_len]:
            hi = mid - 1
        else: 
            return mid
    return -1
