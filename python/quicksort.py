...
class Quicksort(SuffixSorter):
    ...
    def partition(self, start: int, end: int) -> int:
        ...
        pivotIndex = self.pivotSelector.pivotIndex(self.sa, start, end - 1)
        ...
        while True:
            # CompareSuffixes returns 1 if we should swap: Therfore == -1 to stay in loop
            while lo <= hi and compareSuffixes(index[lo], index[pivotIndex]) == -1:
                lo += 1

            # CompareSuffixes returns 1 if we should swap: Therfore == -1 to stay in loop
            while lo <= hi and compareSuffixes(index[pivotIndex], index[hi]) == -1:
                hi -= 1
